...
async function removeData() {
    let dataFile = await readFile('./app/public/data/cols.json')
    ...
    data = data.map(d => {
        ...
        delete d.encoded
        delete d.distance
        delete d.total_elevation_gain
        delete d.elevation_low
        delete d.elevation_high
        delete d.start_latlng
        delete d.end_latlng
        delete d.average_grade

        d.file = d.name.replace(/ /g, '_').toLowerCase()
    ...
    await writeFile('./app/public/data/cols.json', JSON.stringify(data));
}
